...
class MoveJ_ActionServer : public rclcpp::Node
{
    ...
    private:
        ...
        void execute(const std::shared_ptr<GoalHandleMoveJ> goal_handle)
        {
            ...
            move_group_interface.setMaxAccelerationScalingFactor(10.0);
            ...
            if (_limitError == false)
            {
                ...
                // Set up moveit joint model group
                RCLCPP_INFO(this->get_logger(), "Attempting to get Joint Model Group...");

                const moveit::core::JointModelGroup* joint_model_group = move_group_interface.getCurrentState()->getJointModelGroup("ur3e_arm");

                RCLCPP_INFO(this->get_logger(), "Successfully got Joint Model Group...");
                ...
                if(success) {
                    ...
                    RCLCPP_INFO(this->get_logger(), "Planning for MoveJ Action SUCCESSFUL!");
                    move_group_interface.move();

                    // If goal is cancelled
                    if (goal_handle->is_canceling()) {
                        RCLCPP_INFO(this->get_logger(), "MoveJ Action Goal Cancelled");
                        result->result = "MoveJ:CANCELLED";
                        goal_handle->canceled(result);
                        return;
                    } else {
                        RCLCPP_INFO(this->get_logger(), "Movement for MoveJ Action EXECUTED!");
                        result->result = "MoveJ:SUCCESS";
                        goal_handle->succeed(result);
                    }

                } else {
                    RCLCPP_INFO(this->get_logger(), "Planning for MoveJ Action FAILED");
